...
from example.utils.security import hash_password, sign_jwt
...
class AuthService:
    ...
    async def register(self, dto: RegisterDto) -> dict:
        ...
        user = await self.db.fetchrow(
            "INSERT INTO users (name, email, password_hash, roles) VALUES ($1, $2, $3, $4) RETURNING id, name, email, roles, created_at",
            dto.name, dto.email, hash_password(dto.password), ["user"]
        )
        return dict(user)
